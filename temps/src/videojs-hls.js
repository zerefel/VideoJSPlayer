...
videojs.Hls.prototype.selectPlaylist = function () {
  ...
  while (i--) {
   ...
   if (this.player().manualOverride = true) {
       
      switch(this.player().currentQuality) {
           case 1080: presumedWidth = 1920;
           break;
           case 720: presumedWidth = 1280;
           break;
           case 480: presumedWidth = 640;
           break;
       }
       
       for (var i = 0; i < sortedPlaylists.length; i++) {
          
           if (sortedPlaylists[i].attributes.RESOLUTION.height == this.player().currentQuality) {
               return sortedPlaylists[i];
           }
           
           if (!sortedPlaylists[i].attributes.RESOLUTION.height) {
               if (sortedPlaylists[i].attributes.RESOLUTION.width == presumedWidth) {
                   return sortedPlaylists[i];                   
               }
           }
       }
